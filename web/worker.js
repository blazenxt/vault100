...
self.loadArgon2WasmBinary = () =>
  fetch("vendor/argon2.wasm?v=221").then((r) => {
...
async function runJob(msg) {
  ...
  try {
    ...
    if (op === "bench") {
      const res = await VaultFormat.runBench();
      postMessage({ type: "bench", id, res });
      return;
    }
    if (op === "info") {
    ...
    if (op === "encrypt") {
      const res = await VaultFormat.encryptVault(msg.file, {
        ...
        cascade: !!msg.cascade, compress: !!msg.compress,
        metaBaseName: msg.file.name,
